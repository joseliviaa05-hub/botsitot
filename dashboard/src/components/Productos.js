...
function Productos() {
  ...
  const cargarProductos = async () => {
    try {
      ...
      // ✅ FIX: Verificar si res.data tiene la propiedad 'productos'
      if (res.data.productos && Array.isArray(res.data.productos)) {
        productosArray = res.data.productos;
      ...
      const categoriasUnicas = [...new Set(productosArray.map(p => p.categoria))];
  ...
  const formatearPrecio = (producto) => {
    if (producto.precio) {
      return `$${producto.precio.toLocaleString('es-AR')}${producto.unidad ? ' ' + producto.unidad : ''}`;
    } else if (producto.precio_desde) {
      return `Desde $${producto.precio_desde.toLocaleString('es-AR')}`;
    }
    return 'Consultar';
  };
