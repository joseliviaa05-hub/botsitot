...
class OrderController {
    ...
    /**
     * ✅ NUEVO: Actualiza un pedido completo (para marcar como completado)
     */
    async updatePedido(req, res, next) {
        try {
            ...
            logger.info(`📝 Actualizando pedido ${id} con datos:`, req.body);
            
            // Si solo viene "estado", usar la función de actualizar estado
            if (estado && Object.keys(req.body).length === 1) {
                const estadosValidos = ['confirmado', 'preparando', 'enviado', 'entregado', 'completado', 'cancelado'];
                ...
                logger.info(`✅ Pedido ${id} actualizado a: ${estado}`);
            ...
            // Si vienen más campos, actualizar todo
            const pedido = orderService.actualizar(id, req.body);
            
            logger.info(`✅ Pedido ${id} actualizado completamente`);
